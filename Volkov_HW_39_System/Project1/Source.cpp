...
VOID CALLBACK TimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime) // функция для таймера
{
	static time_t t;
...
BOOL CALLBACK DlgProc(HWND hWnd, UINT mes, WPARAM wp, LPARAM lp)
{
	...
	case WM_COMMAND:
	{
		SetTimer(hWnd, 2, 1000, TimerProc); // устанавливаем таймер для обновление ежесекундно списка
